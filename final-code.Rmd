...
load("/Users/irisgao/Desktop/AFM 423/Proj/GP2/data_ml.RData") 
...
```
The graph shows a relationship between log(lambda) and its corresponding MSE. The lowest lambda we got is 5.685894e-05\
...
Prepare the data needed for NN after LASSO
```{r, message = FALSE, warning = FALSE}
...
LASSO + NN hyperparameter Tuning
```{r, message = FALSE, warning = FALSE}
...
model <- keras_model_sequential() %>%
  layer_dense(units = 64, activation = "relu", input_shape = ncol(X_train_nn_lasso)) %>%
...
cat("Neural Network Train RMSE:", rmse(y_train_nn_lasso, pred_nn_lasso_train), "\n")
cat("Neural Network Test RMSE:", rmse(y_test_nn_lasso, pred_nn_lasso_test), "\n")

```

```{r}
plot(y_test_nn, pred_nn_lasso_test,
     xlab = "Actual Return",
...
tune SPCA hyperparameter K and para
...
spca_result <- suppressMessages(spca(X_spca_train, 
...
pred_spca_test <- model_spca %>% predict(Z_test)
...
cat("SPCA + NN Test RMSE:", rmse(y_test_nn, pred_spca_test), "\n")

```

```{r}
plot(y_test_nn, pred_spca_test,
     xlab = "Actual Return",
     ylab = "Predicted Return",
